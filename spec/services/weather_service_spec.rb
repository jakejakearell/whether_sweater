  ...
  describe 'Happy Paths' do
    ...
    it 'will establish a connection to the geocoding service' do
      VCR.use_cassette('weather_service_test') do
        ...
        expect(service[:current][:temp]).to be_a_kind_of(Numeric)
        expect(service[:current]).to have_key(:feels_like)
        expect(service[:current][:feels_like]).to be_a_kind_of(Numeric)
        ...
        expect(service[:current][:uvi]).to be_a_kind_of(Numeric)
